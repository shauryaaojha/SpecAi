...
const callAPI = async (messages, system, maxTokens = 2000) => {
  try {
    const response = await window.puter.ai.chat(
      ...
      { model: "claude-opus-4-6", max_tokens: maxTokens }
    );
    return response.message.content[0].text;
  } catch (err) {
    console.error("Puter API Error:", err);
    return "Error calling AI. Please check console.";
  }
};
...
export default function SpecAI() {
  ...
  const callDiscovery = async (hist) => {
    ...
    if (!parsed) return { phase: "questioning", question: "Can you tell me more about the core problem this app solves?", insight: "Processing..." };
    return parsed;
  };
  ...
  const sendAnswer = async () => {
    ...
    if (result.phase === "complete") {
      setLoading(false);
      setSummary(result.summary || idea);
      setMessages(prev => [...prev, { type: "ai", text: `Got everything! Generating your full spec for "${result.appName}"...`, insight: "Spec complete — building outputs" }]);
      await generateOutputs(result.summary || idea, result.appName || "Your App");
    } else {
